    ...
    >
      <template v-slot:[`item.actions`]="{ item }">
        <v-icon small class="mr-2" @click="editSupplier(item.id)"
          >mdi-pencil</v-icon
        >
        <v-icon small class="mr-2" @click="deleteSupplier(item.id)"
          >mdi-delete</v-icon
        >
        <v-icon small class="mr-2" @click="infoSupplier(item.id)">
    ...
    <template>
      ...
      >
        <v-card>
          ...
          <v-card-subtitle class="mt-2"> Name </v-card-subtitle>
          <v-card-text>
            {{ supplier_info.name }}
          </v-card-text>
          <v-card-subtitle class="mt-2"> Phone Number </v-card-subtitle>
          <v-card-text>
            {{ supplier_info.phone_number }}
          </v-card-text>
          <v-card-subtitle class="mt-2"> Address </v-card-subtitle>
          <v-card-text>
            {{ supplier_info.address }}
          </v-card-text>
          <v-card-subtitle class="mt-2"> Joined At </v-card-subtitle>
          <v-card-text>
          ...
          <v-card-actions>
            <v-spacer></v-spacer>
            <v-btn color="warning" text @click="editSupplier(supplier_info.id)">
              Edit
            </v-btn>
